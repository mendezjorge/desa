  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SKN"."I020234_P" is
...
select SEQ_ID,
 ...
 where collection_name = 'DETALLE_P';
...
SELECT SEQ_ID,
  ...
  FROM APEX_COLLECTIONS
  WHERE COLLECTION_NAME = 'DETA_PAGO_P';
...
--RAISE_aPPLICATION_ERROR(-20001,rtrim(ltrim(upper(V('P_PARA_INIC')))) );
     ...
     parameter.p_clie_prov  := 'P';
     p_emit_reci  := 'R';
     p_clie_prov  := 'P';
     parameter.p_codi_conc  := parameter.p_codi_conc_pgncrr;
     parameter.p_codi_timo  := parameter.p_codi_timo_pgncrr;
    -- set_item_property('bfp.pago_tipo',lov_name,'lov_fp_reci');
...
----actualizar moco.... 
  ...
  v_moco_nume_item := 0;
  v_moco_conc_codi := parameter.p_codi_conc_adlr;--p_codi_conc;
     ...
     if bdet.s_marca = 'S' then
       if bdet.impo_pago > 0 then
          insert into come_movi_cuot_canc(
           ...
           parameter.p_codi_base, 
           round(((nvl(bcab.movi_tasa_mone,0)-nvl(bdet.cuot_tasa_mone,0))*bdet.impo_pago),0)
...
begin
  ...
  BCAB.P_EMIT_RECI              := V('P122_P_EMIT_RECI');
  BCAB.P_CANT_DECI_MMNN         := V('P122_P_CANT_DECI_MMNN');
  BCAB.BUSCAR                   := V('P122_BUSCAR');
  BCAB.P_CLIE_PROV              := V('P122_P_CLIE_PROV');
    
  BCAB.MOVI_SUCU_CODI_ORIG      := V('P122_MOVI_SUCU_CODI_ORIG');
  BCAB.INDI_CARG_DETA           := V('P122_INDI_CARG_DETA');
  BCAB.MOVI_SUCU_DESC_ORIG      := V('P122_MOVI_SUCU_DESC_ORIG');
  BCAB.MOVI_MONE_CANT_DECI      := V('P122_MOVI_MONE_CANT_DECI');
 
  BCAB.MOVI_CLPR_CODI           := V('P122_MOVI_CLPR_CODI');
   
  BCAB.S_CLPR_CODI_ALTE         := V('P122_S_CLPR_CODI_ALTE');
 
  BCAB.MOVI_CODI                := V('P122_MOVI_CODI');
  
  BCAB.MOVI_CLPR_DESC           := V('P122_MOVI_CLPR_DESC');

  BCAB.MOVI_CODI_ADEL           := V('P122_MOVI_CODI_ADEL');
   
  BCAB.MOVI_FECH_EMIS           := V('P122_MOVI_FECH_EMIS');
 
  BCAB.MOVI_TIMO_CODI           := V('P122_MOVI_TIMO_CODI');
  BCAB.MOVI_FECH_OPER           := V('P122_MOVI_FECH_OPER');
  BCAB.MOVI_TIMO_DESC           := V('P122_MOVI_TIMO_DESC');
  BCAB.MOVI_NUME                := V('P122_MOVI_NUME');

   
  BCAB.MOVI_TIMO_DESC_ABRE      := V('P122_MOVI_TIMO_DESC_ABRE');
  BCAB.MOVI_TICO_CODI           := V('P122_MOVI_TICO_CODI');
  BCAB.MOVI_MONE_CODI           := V('P122_MOVI_MONE_CODI');
  BCAB.TIMO_INDI_APLI_ADEL_FOPA := V('P122_TIMO_INDI_APLI_ADEL_FOPA');
  BCAB.TICA_CODI                := V('P122_TICA_CODI');
  BCAB.TIMO_TICA_CODI           := V('P122_TIMO_TICA_CODI');
  
  BCAB.S_DIFE                   := nvl(V('P122_S_DIFE'),0);
    
  BCAB.TIMO_DBCR_CAJA           := V('P122_TIMO_DBCR_CAJA');
  
  BCAB.MOVI_TARE_CODI           := V('P122_MOVI_TARE_CODI');
  BCAB.MOVI_MONE_DESC_ABRE      := V('P122_MOVI_MONE_DESC_ABRE');
   
 
  BCAB.CLPR_PROV_RETENER        := V('P122_CLPR_PROV_RETENER');
  BCAB.TICA_DESC                := V('P122_TICA_DESC');
  BCAB.MOVI_CLPR_RUC            := V('P122_MOVI_CLPR_RUC');
  BCAB.MOVI_CLPR_TELE           := V('P122_MOVI_CLPR_TELE');
  BCAB.MOVI_CLPR_DIRE           := V('P122_MOVI_CLPR_DIRE');
  BCAB.MOVI_MONE_DESC           := V('P122_MOVI_MONE_DESC');
  BCAB.MOVI_TASA_MONE           := nvl(V('P122_MOVI_TASA_MONE'),0);
  BCAB.MOVI_CUEN_NUME           := V('P122_MOVI_CUEN_NUME');
  BCAB.MOVI_TASA_MMEE           := nvl(V('P122_MOVI_TASA_MMEE'),0);
  BCAB.MOVI_FECH_GRAB           := V('P122_MOVI_FECH_GRAB');
  BCAB.S_IMPO_EFEC              := V('P122_S_IMPO_EFEC');
  BCAB.MOVI_GRAV_MMNN           := nvl(V('P122_MOVI_GRAV_MMNN'),0);
  BCAB.MOVI_IVA_MMNN            := nvl(V('P122_MOVI_IVA_MMNN'),0);
  BCAB.MOVI_EXEN_MONE           := nvl(V('P122_MOVI_EXEN_MONE'),0);
  BCAB.MOVI_GRAV_MMEE           := nvl(V('P122_MOVI_GRAV_MMEE'),0);
  BCAB.MOVI_EXEN_MMNN           := nvl(V('P122_MOVI_EXEN_MMNN'),0);
  BCAB.MOVI_EXEN_MMEE           := nvl(V('P122_MOVI_EXEN_MMEE'),0);
  BCAB.S_IMPO_CHEQ              := nvl(V('P122_S_IMPO_CHEQ'),0);
  BCAB.MOVI_IVA_MMEE            := nvl(V('P122_MOVI_IVA_MMEE'),0);
  BCAB.MOVI_GRAV_MONE           := nvl(V('P122_MOVI_GRAV_MONE'),0);
  BCAB.MOVI_OBSE                := V('P122_MOVI_OBSE');
  BCAB.MOVI_IVA_MONE            := nvl(V('P122_MOVI_IVA_MONE'),0);
  BCAB.MOVI_SALD_MONE           := nvl(V('P122_MOVI_SALD_MONE'),0);
  BCAB.MOVI_SALD_MMEE           := nvl(V('P122_MOVI_SALD_MMEE'),0);
  BCAB.MOVI_SALD_MMNN           := nvl(V('P122_MOVI_SALD_MMNN'),0);
  BCAB.MOVI_AFEC_SALD           := V('P122_MOVI_AFEC_SALD');
  BCAB.MOVI_DBCR                := V('P122_MOVI_DBCR');
  BCAB.MOVI_USER                := V('P122_MOVI_USER');
  BCAB.MOVI_EMIT_RECI           := V('P122_MOVI_EMIT_RECI');
  BCAB.CLPR_INDI_CLIE_PROV      := V('P122_CLPR_INDI_CLIE_PROV');
  BCAB.S_IMPO_TARJ              := V('P122_S_IMPO_TARJ');
  BCAB.S_IMPO_ADEL              := V('P122_S_IMPO_ADEL');
...
-- raise_application_error(-20001,  bcab.movi_clpr_codi||'--'||v_impo_pago);
 ...
 v_total := BCAB.S_IMPO_EFEC +BCAB.S_IMPO_CHEQ    ;          
  ...
  if v_clie_dife >0 then
    raise_application_error(-20001,'Atenci?n ! El codigo de cliente ha cambiado, debe cancelar la operacion !!');
  end if;
...
--------------pp_valida_campo_marcado
  ...
  -------pp_actualizar_moimpo;--------se utiliza la forma de pago estandar
...
--raise_application_error(-20001,'Debe ingresar la dddddd del recibo!.');
     APEX_COLLECTION.CREATE_OR_TRUNCATE_COLLECTION(p_collection_name =>'DETALLE_P');
    APEX_COLLECTION.CREATE_OR_TRUNCATE_COLLECTION(p_collection_name =>'DETA_PAGO_P');
...
procedure pp_buscar_moneda (p_movi_mone_codi   in number,
                            ...
                            p_indi_carg_deta      out varchar2) is   
     ...
     for i in c_dica(x.cuot_movi_codi, x.cuot_fech_venc) loop
      ...
      if v_cuot_tasa_mone is null then
       v_cuot_tasa_mone := x.movi_tasa_mone;
           ...
           apex_collection.add_member(p_collection_name => 'DETALLE_P',   
                                      p_c001            => x.movi_nume,
...
v_saldo_dif number;
 ...
 APEX_COLLECTION.UPDATE_MEMBER_ATTRIBUTE(P_COLLECTION_NAME    => 'DETALLE_P',
                                            P_SEQ             => p_seq_id,
                                            P_ATTR_NUMBER     => 12,
   ...
   v_saldo := null;
     ...
     where collection_name = 'DETALLE_P'
       and seq_id = p_seq_id;   
        ...
        if nvl(V('P122_MOVI_EXEN_MONE'),0) <> 0 then
          if V('P122_S_DIFE') < 0 then
        	  raise_application_error(-20001,'El importe de la distribucion de pagos es mayor al importe del recibo');
          end if;	 
        
          if v_saldo <= V('P122_S_DIFE') then
              v_impo_pago := v_saldo;
            else
        	    v_impo_pago := V('P122_S_DIFE');
            end if;	  
        end if;  
             
        if v_movi_fech_emis > V('P122_movi_fech_emis') then
...
APEX_COLLECTION.UPDATE_MEMBER_ATTRIBUTE(P_COLLECTION_NAME   => 'DETALLE_P',
  ...
  v_saldo_dif := nvl(v('P122_MOVI_EXEN_MONE'),0)-v_saldo_tot;
...
v_impo_mmnn	number;
v_impo_movi	number;
v_pago_impo_movi1 varchar2(1000);
v_pago_impo_mone1 varchar2(1000);
v_pago_impo_mmnn1 varchar2(1000);
v_cheq_cuen_codi varchar2(1000);
...
if p_pago_tipo is null then
 ...
 END IF;
  ...
  if p_mone_codi = 1 then
   ...
   v_pago_impo_mmnn:=  round(v_pago_impo_mmnn,parameter.p_cant_deci_mmnn);
    ...
    if p_mone_codi <> V('P122_MOVI_MONE_CODI') then
        if  V('P122_MOVI_MONE_CODI') = 1 then
          v_impo_movi := p_pago_impo_mone* P_pago_coti;
        else
          if p_mone_codi = 1 then
            v_impo_movi := p_pago_impo_mone/V('P122_MOVI_TASA_MONE');
          else
            v_impo_mmnn := p_pago_impo_mone* p_pago_coti;
            v_impo_movi := v_impo_mmnn / V('P122_MOVI_TASA_MONE');
          end if;
        end if;
      else
      ...
      v_pago_impo_movi := round(v_impo_movi, V('P122_MOVI_MONE_CANT_DECI'));
 ...
 apex_collection.add_member(p_collection_name => 'DETA_PAGO_P',  
                            P_C001      => P_PAGO_TIPO,
...
--raise_application_error(-20001,p_pago_tipo||'--'||V('P122_P_EMIT_RECI') );
...
begin
  ...
  if V('P122_EMIT_RECI') = 'R' then
    if p_pago_tipo = '1' then
      if V('P122_TIMO_DBCR_CAJA') = 'C' then
        p_pago_cobr_indi := 'C';
      else
        p_pago_cobr_indi := 'D';
      end if;
...
if p_mone_codi is not null then
   ...
   if p_mone_codi = V('P122_MOVI_MONE_CODI') then
     p_pago_coti := V('P122_MOVI_TASA_MONE');
       --  pp_habi_desh_sald_mone('D',null, null, null);
   else
     pp_busca_tasa_mmee (p_mone_codi, p_pago_coti,V('P122_TIMO_TICA_CODI'),V('P122_MOVI_FECH_EMIS'));
...
begin
  ...
  if parameter.p_codi_mone_mmnn = p_mone_codi then
    ...
    where coti_mone    = p_mone_codi
    and coti_tica_codi = p_tica_codi
    and coti_fech      = V('P122_MOVI_FECH_EMIS');
  end if;   
...
begin
  ...
  if p_pago_tipo in ('1','2','3','4','5','8') then
    if P_pago_cuen_codi is not null then
        ...
        if V('P122_MOVI_DBCR') = 'D' then
          if not general_skn.fl_vali_user_cuen_banc_debi (p_pago_cuen_codi,  V('P122_MOVI_FECH_EMIS')) then
             raise_application_error(-20001,'No posee permiso para ingresar en la caja seleccionada. !!!');
          end if;
        else      
          if not general_skn.fl_vali_user_cuen_banc_cred (p_pago_cuen_codi, V('P122_MOVI_FECH_EMIS')) then
              raise_application_error(-20001,'No posee permiso para ingresar en la caja seleccionada. !!!');
          end if;
        end if;
...
*/
  --I020234.PP_VALIDA_NUME_CHEQ(:P122_CHEQ_NUME, :P122_CHEQ_SERIE, :P100_CHEQ_BANC_CODI); 
...
begin
 ...
 -- pago
  p_tota_fact := NVL(V('P122_MOVI_EXEN_MONE'),0);
  p_tota_efec_egre :=0;
  p_tota_cheq_egre :=0;
  p_tota_efec_ingr :=0;
  p_tota_cheq_ingr :=0;
...
--raise_application_error(-20001,Nvl(V('P122_TOTA_FACT'),0)||'--'||nvl(p_tota_pago,0));
...
SETITEM('P122_S_IMPO_EFEC', p_tota_pago);
SETITEM('P122_S_IMPO_CHEQ', p_tota_sald);


 if V('P122_EMIT_RECI') = 'E'  then
    if V('P122_TIMO_DBCR_CAJA') = 'C' then
      SETITEM('P122_S_IMPO_EFEC',  -nvl(p_tota_efec_ingr,0) + nvl(p_tota_efec_egre,0) );
      SETITEM('P122_S_IMPO_CHEQ',  -nvl(p_tota_cheq_ingr,0) + nvl(p_tota_cheq_egre,0) );
 ...
 elsif V('P122_EMIT_RECI') = 'R'  then                                         
    if V('P122_TIMO_DBCR_CAJA')= 'C' then
      SETITEM('P122_S_IMPO_EFEC',  -nvl(p_tota_efec_ingr,0) + nvl(p_tota_efec_egre,0)); /*- nvl(:brete.movi_impo_rete, 0)*/
      SETITEM('P122_S_IMPO_CHEQ',  nvl(p_tota_cheq_egre,0) - nvl(p_tota_cheq_ingr,0));
    else
      SETITEM('P122_S_IMPO_EFEC', nvl(p_tota_efec_ingr,0) - nvl(p_tota_efec_egre,0));
      SETITEM('P122_S_IMPO_CHEQ', -nvl(p_tota_cheq_egre,0) + nvl(p_tota_cheq_ingr,0));
    end if; 
...
END LOOP;
 ...
 elsif V('P122_P_EMIT_RECI') = 'R'  then                                         
    if V('P122_TIMO_DBCR_CAJA')= 'C' then
      SETITEM('P122_S_IMPO_EFEC',  -nvl(p_tota_efec_ingr,0) + nvl(p_tota_efec_egre,0)); /*- nvl(:brete.movi_impo_rete, 0)*/
      SETITEM('P122_S_IMPO_CHEQ',  nvl(p_tota_cheq_egre,0) - nvl(p_tota_cheq_ingr,0));
    else
 ...
 --SETITEM('P122_S_IMPO_EFEC', 1 - 2);
  --    SETITEM('P122_S_IMPO_CHEQ',2);
