  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SKN"."I010068" is

  procedure pa_abm_barr(v_ind            in varchar2,
  ...
  begin
    ...
    if v_ind = 'I' then
      select nvl(max(barr_codi), 0) + 1,
             nvl(max(to_number(barr_codi_alte)), 0) + 1
        into x_barr_codi, x_barr_codi_alte
        from come_barr;
  ...
  ---------------------------------------------------------------------------------------
  procedure pa_elim_regi(v_barr_codi in number) is
  begin

    I010068.pa_vali_elim_regi(v_barr_codi);
    delete come_barr where barr_codi = v_barr_codi;

  end pa_elim_regi;

  ---------------------------------------------------------------------------------------
  procedure pa_vali_elim_regi(v_barr_codi in number) is
    v_count number(6);
  begin
    select count(*)
      into v_count
