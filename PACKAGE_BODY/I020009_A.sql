  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SKN"."I020009_A" is
  ...
  type t_bcab is record(
    ...
    movi_codi_rete           come_movi.movi_codi_rete%type,
    ...
    clpr_indi_clie_prov varchar2(2),
    clpr_prov_retener varchar2(2),
    s_calc_rete varchar2(2),
    s_timo_calc_iva varchar2(2),
    s_calc_rete_mini varchar2(2),
    s_calc_rete_ivae varchar2(2),
    clpr_pais_codi varchar2(2),
    s_calc_rete_rent varchar2(2),
    TIMO_INDI_CLPR varchar2(2),
    clpr_zona_codi number,
...
tota_viaj_egre_movi number);
 bfp t_bfp;
  ...
  type t_brete is record(
    ...
    s_abso_rete_rent         varchar2(1),
    --
   tota_impo_rete number,
   movi_codi_rete number,
  s_nro_1_rete number,
  w_nro_1_ante number,
  s_nro_2_rete number,
  w_nro_2_ante number,
  ...
  if   bcab.MOVI_TICL_CODI is null and (v('P74_MOVI_RE90_TIPO') = 'S'
    or v('P74_MOVI_RE90_CODI') = 'S' or ('P74_MOVI_TICL_CODI') = 'S'
    or v('P74_MOVI_INDI_IMPU_IVA') = 'S'  or v('P74_MOVI_INDI_IMPU_IRE') = 'S'
     or v('P74_MOVI_INDI_IMPU_IRP_RSP') = 'S'
     or v('P74_MOVI_INDI_NO_IMPU') = 'S' )then

  raise_application_error(-20010, 'Tipo docu no puede quedar nulo');
  end if;

   if  bcab.movi_timo_codi   is null then
    raise_application_error(-20010, 'El tipo de movimiento  no puede quedar vacio');
   end if;
...
if bcab.movi_fech_oper  < bcab.movi_fech_emis then
  raise_application_error(-20010., 'La Fecha de Operacion no puede ser inferior a la Fecha de Emision');
          ...
          I020231.PP_VALIDAR_NRO(I_TICO_FECH_REIN      => v('P74_TICO_FECH_REIN'),
                                 I_MOVI_FECH_EMIS      => bcab.movi_fech_emis,
                                 I_TICO_CODI           => v('P74_TICO_CODI'),
                                 I_CLPR_INDI_CLIE_PROV => v('P74_TIMO_INDI_CLPR'),
                                 I_MOVI_CLPR_CODI      => v('P74_MOVI_CLPR_CODI'),
                                 I_MOVI_NUME           => v('P74_MOVI_NUME'));

          --I020009_A.pp_validar_nro2;
          
        if V('P74_TICO_CODI') is not null and  V('P74_TICO_INDI_VALI_TIMB') ='S'then
        if  bcab.MOVI_NUME_TIMB           is null then
              raise_application_error(-20010, 'El timbrado no puede quedar vacio'|| bcab.MOVI_NUME_TIMB );
         end if;
         if  bcab.fech_venc_timb          is null then
             raise_application_error(-20010, 'La fecha vto de timbrado no puede quedar vacio');
         end if;

    /*      I020231.PP_VALIDAR_TIMBRADO(P_TICO_CODI           =>V('P74_TICO_CODI'),
                                      p_esta                =>v('P74_S_NRO_1'),
                                      P_PUNT_EXPE           =>V('P74_S_NRO_2'),
                                      p_clpr_codi           =>v('P74_MOVI_CLPR_CODI'),
                                      P_FECH_MOVI           => bcab.movi_fech_emis,
                                      p_timb                =>P_TIMB,
                                      P_FECH_VENC           =>p_fech_venc,
                                      p_tico_indi_timb      =>v('P74_TICO_INDI_TIMB'),
                                      I_CLPR_INDI_CLIE_PROV =>V('P74_TIMO_INDI_CLPR'),
                                      i_s_clpr_codi_alte    =>v('P74_CLPR_CODI_ALTE'),
                                      I_TICO_INDI_TIMB_AUTO =>V('P74_tico_indi_timb_auto'),
                                      i_tico_indi_vali_timb =>v('P74_TICO_INDI_VALI_TIMB'),
...
*/
  ...
  begin
    ...
    bcab.movi_codi                := v('P74_MOVI_CODI');
    bcab.movi_clpr_codi           := v('P74_MOVI_CLPR_CODI');
    bcab.movi_timo_indi_sald      := v('P74_MOVI_TIMO_INDI_SALD');
    bcab.movi_afec_sald           := v('P74_MOVI_AFEC_SALD');
    bcab.movi_timo_indi_adel      := v('P74_MOVI_TIMO_INDI_ADEL');
    bcab.movi_timo_indi_ncr       := v('P74_MOVI_TIMO_INDI_NCR');
    bcab.timo_dbcr_caja           := v('P74_TIMO_DBCR_CAJA');
    bcab.movi_timo_codi           := v('P74_MOVI_TIMO_CODI');
    bcab.movi_clpr_codi           := v('P74_MOVI_CLPR_CODI');
    bcab.movi_sucu_codi_orig      := v('P74_MOVI_SUCU_CODI_ORIG');
    bcab.movi_mone_codi           := v('P74_MOVI_MONE');
    bcab.movi_nume                := v('P74_MOVI_NUME');
    bcab.movi_fech_emis           := v('P74_MOVI_FECH_EMIS');
    bcab.movi_fech_oper           := v('P74_MOVI_FECH_OPER');
    bcab.movi_fech_grab           := v('P74_MOVI_FECH_GRAB');
    bcab.movi_user                := v('P74_MOVI_USER');
    bcab.movi_tasa_mone           := v('P74_MOVI_TASA_MONE');
    bcab.movi_grav_mmnn           := v('P74_MOVI_GRAV_MMNN');
    bcab.movi_exen_mmnn           := v('P74_MOVI_EXEN_MMNN');
    bcab.movi_iva_mmnn            := v('P74_MOVI_IVA_MMNN');
    bcab.movi_grav_mone           := v('P74_MOVI_GRAV_MONE');
    bcab.movi_exen_mone           := v('P74_MOVI_EXEN_MONE');
    bcab.movi_iva_mone            := v('P74_MOVI_IVA_MONE');
    bcab.movi_obse                := v('P74_MOVI_OBSE');
    bcab.movi_sald_mmnn           := v('P74_MOVI_SALD_MMNN');
    bcab.movi_sald_mone           := v('P74_MOVI_SALD_MONE');
    bcab.movi_stoc_suma_rest      := v('P74_MOVI_STOC_SUMA_REST');
    bcab.movi_clpr_dire           := v('P74_MOVI_CLPR_DIRE');
    bcab.movi_clpr_tele           := v('P74_MOVI_CLPR_TELE');
    bcab.movi_clpr_ruc            := v('P74_MOVI_CLPR_RUC');
    bcab.movi_clpr_desc           := v('P74_MOVI_CLPR_DESC');
    bcab.movi_emit_reci           := v('P74_MOVI_EMIT_RECI');
    bcab.movi_afec_sald           := v('P74_MOVI_AFEC_SALD');
    bcab.movi_dbcr                := v('P74_MOVI_DBCR');
    bcab.movi_stoc_afec_cost_prom := v('P74_MOVI_STOC_AFEC_COST_PROM');
    bcab.movi_empr_codi           := v('P74_MOVI_EMPR_CODI');
    bcab.movi_empl_codi           := v('P74_MOVI_EMPL_CODI');
    bcab.movi_nume_timb           := v('P74_MOVI_NUME_TIMB');
    bcab.fech_venc_timb           := v('P74_MOVI_FECH_VENC_TIMB');
    bcab.movi_impo_mone_ii        := v('P74_MOVI_IMPO_MONE_II');
    bcab.movi_impo_mmnn_ii        := v('P74_MOVI_IMPO_MMNN_II');
    bcab.movi_grav10_ii_mone      := v('P74_MOVI_GRAV10_II_MONE');
    bcab.movi_grav5_ii_mone       := v('P74_MOVI_GRAV5_II_MONE');
    bcab.movi_grav10_ii_mmnn      := v('P74_MOVI_GRAV10_II_MMNN');
    bcab.movi_grav5_ii_mmnn       := v('P74_MOVI_GRAV5_II_MMNN');
    bcab.movi_grav10_mone         := v('P74_MOVI_GRAV10_MONE');
    bcab.movi_grav5_mone          := v('P74_MOVI_GRAV5_MONE');
    bcab.movi_grav10_mmnn         := v('P74_MOVI_GRAV10_MMNN');
    bcab.movi_grav5_mmnn          := v('P74_MOVI_GRAV5_MMNN');
    bcab.movi_iva10_mone          := v('P74_MOVI_IVA10_MONE');
    bcab.movi_iva5_mone           := v('P74_MOVI_IVA5_MONE');
    bcab.movi_iva10_mmnn          := v('P74_MOVI_IVA10_MMNN');
    bcab.movi_iva5_mmnn           := v('P74_MOVI_IVA5_MMNN');
    bcab.movi_indi_apli_tick      := v('P74_MOVI_INDI_APLI_TICK');

    bcab.movi_ticl_codi         := v('P74_MOVI_TICL_CODI');
    bcab.movi_re90_tipo         := v('P74_MOVI_RE90_TIPO');
    bcab.movi_re90_codi         := v('P74_MOVI_RE90_CODI');
    bcab.movi_indi_impu_iva     := v('P74_MOVI_INDI_IMPU_IVA');
    bcab.movi_indi_impu_ire     := v('P74_MOVI_INDI_IMPU_IRE');
    bcab.movi_indi_impu_irp_rsp := v('P74_MOVI_INDI_IMPU_IRP_RSP');
    bcab.movi_indi_no_impu      := v('P74_MOVI_INDI_NO_IMPU');

    bcab.s_impo_rete      := v('P74_S_IMPO_RETE');
    bcab.s_impo_rete_rent := v('P74_S_IMPO_RETE');
    bcab.s_total          := v('P74_S_TOTAL');

    bcab.s_grav_10_ii := v('P74_S_GRAV_10_II');
    bcab.s_grav_5_ii  := v('P74_S_GRAV_5_II');
    bcab.s_exen       := v('P74_S_EXEN');



  bcab.timo_indi_clpr :=v('P74_TIMO_INDI_CLPR');
  bcab.s_calc_rete    :=v('P74_S_CALC_RETE');
  bcab.s_calc_rete_rent :=v('P74_S_CALC_RETE_RENT');
  bcab.s_abso_rete_rent :=v('P74_S_ABSO_RETE_RENT');
  bcab.s_calc_rete_mini :=v('P74_S_CALC_RETE_MINI');
  bcab.s_clpr_codi_alte :=v('P74_CLPR_CODI_ALTE');

  bcab.movi_empr_codi:=p_empr_codi;

  bcab.movi_mone_cant_deci  := v('P74_MONE_CANT_DECI');
    ...
    ------R E T E N  C I O N
    brete.movi_nume_rete           := v('P74_MOVI_NUME_RETE');
    brete.movi_nume_timb_rete      := v('P74_MOVI_NUME_TIMB_RETE');
    brete.movi_fech_venc_timb_rete := v('P74_MOVI_FECH_VENC_TIMB_RETE');
    brete.movi_fech_rete           := v('P74_S_MOVI_FECH_RETE');
    brete.movi_impo_rete           := v('P74_MOVI_IMPO_RETE');
    brete.movi_impo_rete_rent      := v('P74_MOVI_IMPO_RETE_RENT');
...
/*	  --preguntar si se desea imprimir el reporte...
  ...
  pa_gene_tesaka('COMPRA_'||bcab.movi_nume||'_'||bcab.movi_clpr_desc||'_'||to_char(bcab.movi_fech_emis,'yyyymmdd')||'.txt',
                 null,
                 null,
                 bcab.movi_codi,
                 null,
...
exception
  ...
  procedure pp_actualizar_registro(ii_movi_indi_dist            in varchar2,
    ...
    v_imprimir1 varchar2(1000);
    v_imprimir2 varchar2(1000);
    v_imprimir3 varchar2(1000);
    v_imprimir4 varchar2(1000);
    i_movi_codi number;
  ...
  I020009_A.pp_iniciar;
  pp_set_value;
   pp_muestra_clpr(bcab.timo_indi_clpr,
  ...
  pp_validar_cabecera;
    ...
    if ii_movi_codi is not null then
        I020009_A.pp_validar_asiento(ii_movi_codi);
        I020009_A.pp_borrar_registro(ii_movi_codi);
        i_movi_codi := null;
    end if;
    ...
    begin
        ...
        -- AL RECUPERAR PARA MODIFICAR Y LUEGO QUERER VOLVER SE PIERDE EL TOTAL..
  if ii_movi_timo_codi in (p_codi_timo_auto_fact_emit, p_codi_timo_auto_fact_emit_cr) then
    if p_apli_rete_exen_defe = 'S' and nvl(ii_clpr_indi_clie_prov, 'C') = 'P' then
      s_impo_mone := nvl(ii_s_total,0) - nvl(ii_s_impo_rete,0)  - nvl(ii_s_impo_rete_rent,0);
      s_impo_mmnn := (nvl(ii_s_total,0) - nvl(ii_s_impo_rete,0)  - nvl(ii_s_impo_rete_rent,0)) * ii_movi_tasa_mone;
     else
      s_impo_mone := nvl(ii_s_total,0);
      s_impo_mmnn := nvl(ii_s_total,0) * ii_movi_tasa_mone;
     end if;
  else
    s_impo_mone := nvl(ii_s_total,0) - nvl(ii_s_impo_rete,0)  - nvl(ii_s_impo_rete_rent,0);
    s_impo_mmnn := (nvl(ii_s_total,0) - nvl(ii_s_impo_rete,0)  - nvl(ii_s_impo_rete_rent,0)) * ii_movi_tasa_mone;
      ...
      I020009_A.pp_valida_totales(i_f_dif_impo                => ii_f_dif_impo,
                                ...
                                i_s_totales                 => s_impo_mone,--ii_s_total,
                                i_sum_s_impo_movi           => ii_sum_s_impo_movi);
    end;

    --validamos cheques
    begin
      I020009_A.pp_valida_cheques;

      end;

    --validar importes
    begin
      I020009_A.pp_validar_importes(i_movi_timo_codi      => ii_movi_timo_codi,
                                    i_movi_timo_indi_caja => ii_movi_timo_indi_caja,
                                    i_clpr_indi_clie_prov => ii_clpr_indi_clie_prov,
                                    i_s_total             =>  s_impo_mone);-- ii_s_total);
    ...
    if nvl(bcab.movi_timo_indi_sald, 'N') <> 'N' then
     ...
     select sum(c003) cuot_impo_mone
       into v_monto_cuota
  ...
  -- raise_application_error(-20010,nvl(v_monto_cuota,0)||'--'||nvl(bcab.s_total,0) ||'Diferencia entre el total del Movimientos y el total de las cuotas, Favor Verificar');


      if nvl(bcab.s_total,0) <> nvl(v_monto_cuota,0) then
        raise_application_error(-20010,nvl(v_monto_cuota,0)||'--'||nvl(bcab.s_total,0) ||'Diferencia entre el total del Movimientos y el total de las cuotas, Favor Verificar');
     end if;
    ...
    --validar conceptos
    begin
      I020009_A.pp_validar_conceptos;
    end;

    --ajuste de los importes
    begin
      I020009_A.pp_ajustar_importes;
    end;

    --termina las validaciones y ajustes e iniciamos las inserciones
    --guardamos cabecera
     I020009_A.pp_actualiza_come_movi;
     ...
     I020009_A.pp_actualiza_moco(p_movi_codi);

     I020009_A.pp_actualiza_moimpu(ii_movi_timo_codi   => p_movi_codi,
    ...
    if nvl(bcab.s_impo_rete, 0) > 0 or nvl(bcab.s_impo_rete_rent, 0) > 0 then
        I020009_A.pp_actualizar_rete2;
    end if;

      pp_gene_asie(ii_movi_fech_emis);
  ---se hablo con sol y es mejor no actualizar ya que se suele hacer mucho ese salto de numeraci?n
 /* if   v('P74_TIMO_INDI_CLPR') = 'C' then

  I020009_A.pp_actu_secu(p_indi => 'S');
 end if;*/
    ...
    if p_indi_impr_dbcr = 'S' then
       v_imprimir4 := 'MVAR';
        /*I020009_A.pp_reporte(i_movi_codi      => p_movi_codi,
                            i_movi_timo_codi => ii_movi_timo_codi);*/

    end if;
 --RAISE_APPLICATION_ERROR(-20001,bcab.timo_dbcr_caja||'/'||v('TIMO_INDI_CLPR')||'-'|| bcab.movi_afec_sald );
    --Generamos el PDF para la Orden de Pago
    if bcab.timo_dbcr_caja = 'C' and nvl(ii_clpr_indi_clie_prov, 'C') = 'P' then
      if bcab.movi_afec_sald = 'N' then
        begin
          v_imprimir1 := 'OPAG';
        --  I020009_A.pp_reporte_Orden_pago(i_movi_codi => p_movi_codi);
        end;
      end if;
    end if;


    IF BCAB.MOVI_TIMO_CODI IN (1,2,3,4,9) THEN
       v_imprimir2 := 'FACT';
		  /*PACK_REPO_CLIE.PP_LLAMAR_REPORTE_FACTURAS(P_USUARIO => gen_user,
                                                 P_CODIGO  => p_movi_codi);*/
   ...
   delete from come_tabl_auxi
       where taax_sess = v('APP_SESSION')
         ...
         and nvl(taax_C050,'x')<>'REPORTE';
  --- RAISE_APPLICATION_ERROR(-20001,'HOLA FINAL'||bcab.timo_dbcr_caja||'--'||bcab.movi_afec_sald||'**'||p_movi_codi);
    ...
    */
    ...
    p_indi_rete_tesaka := NVL(v('P74_GENE_TESAKA'),'N');
   ...
   -- RAISE_APPLICATION_ERROR(-20001,'3'||v_imprimir3||'2'||v_imprimir2||'1'||v_imprimir1);
   v_mensaje :='</br> REGISTRO ACTUALIZADO' ;

    IF v_imprimir3 IS NOT NULL THEN

        v_mensaje                                := v_mensaje||'</br>' ||
                                                    '<a href="javascript:$s(''P74_IMPRIMIR_DOCU'',' ||bcab.movi_codi_rete||');javascript:$s(''P74_REPORTE'',''RETE'');">Imprimir Retencion</a> ';
    END IF;
    IF v_imprimir1 IS NOT NULL THEN
        v_mensaje                                := v_mensaje||'</br>' ||
                                                    '<a href="javascript:$s(''P74_IMPRIMIR_DOCU'',' ||bcab.movi_codi||');javascript:$s(''P74_REPORTE'',''OPAG'');">Imprimir Orden Pago</a> ';
    END IF;
    IF v_imprimir2 IS NOT  NULL THEN
       v_mensaje                                := v_mensaje||'</br>' ||
                                                '<a href="javascript:$s(''P74_IMPRIMIR_DOCU'',' ||bcab.movi_codi||');javascript:$s(''P74_REPORTE'',''FACT'');">Imprimir Factura</a> ';

    END IF;
    IF v_imprimir4 IS NOT  NULL THEN
       v_mensaje                                 := v_mensaje||'</br>' ||
                                                '<a href="javascript:$s(''P74_IMPRIMIR_DOCU'',' ||bcab.movi_codi||');javascript:$s(''P74_REPORTE'',''MVAR'');">Imprimir Det. Factura</a> ';
    END IF;
  ...
  procedure pp_actualiza_come_movi_cuot(i_movi_timo_indi_sald in varchar2,
   ...
   /*   select taax_c001 Pago_A,
    ...
    select to_number(c001) cuot_impo_mmnn,
           c002 cuot_fech_venc,
           to_number(c003) cuot_impo_mone,
           c004 Pago_A,
           seq_id
        from apex_collections_full a
       where a.collection_name = 'BCUOTA'
  ...
  begin
    if i_movi_timo_indi_sald = 'S' then
      ...
      for i in c_cuot loop
        ...
        I020009_A.pp_insert_come_movi_cuot(v_cuot_fech_venc,
                                         v_cuot_nume,
                                         v_cuot_impo_mone,
                                         v_cuot_impo_mmnn,
                                         v_cuot_impo_mmee,
  ...
  begin
    ...
    for i in c_conc_deta loop
      ...
      I020009_A.pp_insert_movi_conc_deta(v_moco_movi_codi,
                                       v_moco_nume_item,
                                       v_moco_conc_codi,
                                       v_moco_cuco_codi,
                                       v_moco_impu_codi,
  ...
  */
  ...
  begin
    ...
    if ii_movi_timo_codi in
      ...
      I020009_A.pp_insert_come_movi_impu_deta(v_moim_impu_codi,
                                            v_moim_movi_codi,
                                            v_moim_impo_mmnn,
                                            v_moim_impo_mmee,
                                            v_moim_impu_mmnn,
...
procedure pp_actualizar_rete2 is
  v_movi_codi                number;
  v_movi_timo_codi           number;
  v_movi_clpr_codi           number;
  v_movi_sucu_codi_orig      number;
...
*/
  ...
  if nvl(brete.movi_impo_rete, 0) > 0 then
    ...
    bcab.movi_codi_rete := p_movi_codi_rete;
  ...
  RAISE_aPPLICATION_ERROR(-20001, brete.movi_impo_rete_rent);
  ...
  begin
    ...
    for i in 1 .. ta_bdet.count loop
      ...
      begin
        I020009_A.pp_calcular_importe_item(v_nro_item);
      end;
    ...
    begin
      I020009_A.pp_calcular_importe_cab;
    end;
  ...
  begin
    ...
    bcab.movi_impo_mone_ii := nvl(bcab.movi_grav10_ii_mone,0) +
                              nvl(bcab.movi_grav5_ii_mone,0) + nvl(bcab.movi_exen_mone,0);
    bcab.s_total           := nvl(bcab.movi_impo_mone_ii,0);

    ---importe iva incluido en mmnn

    bcab.movi_grav10_ii_mmnn := round((nvl(bcab.movi_grav10_ii_mone,0) *
                                      bcab.movi_tasa_mone),
                                      p_cant_deci_mmnn);
    bcab.movi_grav5_ii_mmnn  := round((nvl(bcab.movi_grav5_ii_mone,0) *
                                      bcab.movi_tasa_mone),
                                      p_cant_deci_mmnn);
    bcab.movi_exen_mmnn      := round((nvl(bcab.movi_exen_mone,0) *
                                      bcab.movi_tasa_mone),
                                      p_cant_deci_mmnn);

    bcab.movi_iva10_mone := round((nvl(bcab.movi_grav10_ii_mone,0) / 11),
                                  bcab.movi_mone_cant_deci);
    bcab.movi_iva5_mone  := round((nvl(bcab.movi_grav5_ii_mone,0) / 21),
                                  bcab.movi_mone_cant_deci);

    bcab.s_iva_10  := nvl(bcab.movi_iva10_mone,0);
    bcab.s_iva_5   := nvl(bcab.movi_iva5_mone,0);
    bcab.s_tot_iva := nvl(bcab.movi_iva10_mone,0) + nvl(bcab.movi_iva5_mone,0);

    bcab.movi_iva10_mmnn := round((nvl(bcab.movi_grav10_ii_mmnn,0) / 11),
                                  p_cant_deci_mmnn);
    bcab.movi_iva5_mmnn  := round((nvl(bcab.movi_grav5_ii_mmnn,0) / 21),
                                  p_cant_deci_mmnn);

    bcab.movi_grav10_mone := round((nvl(bcab.movi_grav10_ii_mone ,0)/ 1.1),
                                   bcab.movi_mone_cant_deci);
    bcab.movi_grav5_mone  := round((nvl(bcab.movi_grav5_ii_mone,0) / 1.05),
                                   bcab.movi_mone_cant_deci);

    bcab.movi_grav10_mmnn := round((nvl(bcab.movi_grav10_ii_mmnn,0) / 1.1),
                                   p_cant_deci_mmnn);
    bcab.movi_grav5_mmnn  := round((nvl(bcab.movi_grav5_ii_mmnn,0) / 1.05),
                                   p_cant_deci_mmnn);

    bcab.movi_impo_mmnn_ii := nvl(bcab.movi_grav10_ii_mmnn,0) +
                              nvl(bcab.movi_grav5_ii_mmnn,0) + nvl(bcab.movi_exen_mmnn,0);

    --importes netos
    bcab.movi_grav_mone := nvl(bcab.movi_grav10_mone,0) + nvl(bcab.movi_grav5_mone,0);
    bcab.movi_iva_mone  := nvl(bcab.movi_iva10_mone,0) + nvl(bcab.movi_iva5_mone,0);

    bcab.movi_grav_mmnn := nvl(bcab.movi_grav10_mmnn,0) + nvl(bcab.movi_grav5_mmnn,0);
    bcab.movi_iva_mmnn  := nvl(bcab.movi_iva10_mmnn,0) + nvl(bcab.movi_iva5_mmnn,0);
    ...
    if nvl(bcab.movi_timo_indi_sald, 'N') = 'N' then
      ...
      bcab.movi_sald_mone := nvl(bcab.movi_impo_mone_ii,0);
      bcab.movi_sald_mmnn := nvl(bcab.movi_impo_mmnn_ii,0);
    end if;


 -- raise_application_error(-20001, bcab.movi_grav_mmnn ||'-'|| p_cant_deci_mmnn||'-//'|| bcab.movi_mone_cant_deci );
...
begin
    if nvl(p_tico_indi_timb,'C') <> 'S' then
      ...
      select setc_nume_timb, setc_fech_venc
       ...
       where setc_secu_codi =  (select user_secu_codi from segu_user a
...
p_timb   :=v_nume_timb;
p_fech_venc  :=v_fech_venc;

exception
  when no_data_found then
     raise_application_error(-20010,'El proveedor no tiene un timbrado vigente. Favor verificar.');
  when others then
    ...
    cursor c_timb_3 is
      select setc_nume_timb, setc_fech_venc
       ...
       where setc_secu_codi =
         ...
         and setc_punt_expe = p_punt_expe --punto de expedicion
         and setc_fech_venc >= p_fech_movi
         and 'P' =v('P74_TIMO_INDI_CLPR')
         union all

      select setc_nume_timb, setc_fech_venc
        from come_secu_tipo_comp
       where setc_secu_codi =
             (select user_secu_codi from segu_user a
                             where  a.user_login = gen_user)
         and setc_tico_codi = p_tico_codi --tipo de comprobante
         and setc_esta = p_esta --establecimiento
         and setc_punt_expe = p_punt_expe --punto de expedicion
         and setc_fech_venc >= p_fech_movi
         and 'C' = v('P74_TIMO_INDI_CLPR')
       order by setc_fech_venc;
  ...
  if v('P74_MOVI_TIMO_CODI')<> 9 then
   ...
   -- raise_application_error(-20001,  v_timb||'/'||  v('P74_MOVI_TIMO_CODI'));
    ----------------------***---------------------------
    if p_tico_codi is not null and v('P74_MOVI_TIMO_CODI') = 9 then
      -- raise_application_error(-20001,'v_indi_entro');
      if (nvl(upper(i_tico_indi_vali_timb), 'N') = 'S' or  v('P74_MOVI_TIMO_CODI') =9)
      and nvl(upper(i_tico_indi_timb_auto), 'N') = 'S' then
        pp_devu_timb (p_tico_codi,
                      p_esta,
                      p_punt_expe,
                      p_clpr_codi,
                      p_fech_movi,
        ...
        --     raise_application_error(-20001,  v_timb||'/'||  v_vto);
       p_timb      := v_timb;
       p_fech_venc := v_vto;
      end if;
 ...
 --  raise_application_error(-20001,v_indi_entro||'-'||p_tico_codi||'-'||p_punt_expe||'-'||p_esta||'-'||p_fech_movi);
  ...
  begin
    ...
    if o_ortr_fech_emis > v('P74_MOVI_FECH_EMIS') then
      raise_application_error(-20010,
  ...
  */  when others then
  ...
  begin
    raise_application_Error(-20001, i_s_total );
  ...
  begin
    ...
    if i_movi_afec_sald = 'N' then
      if nvl(i_sum_s_impo_movi, 0) <> nvl(i_s_totales, 0) then
        -- si existe diferencia y el documento es en gs. el pago es en otra moneda,
        -- por ende se ajustara en importe nacional si es menor a 100gs
        ...
        raise_application_error(-20010, 'Total FP: '||i_sum_s_impo_movi||' Total Cabecera: '||i_s_totales||
  ...
  begin
    ...
    if v_indi_ot = 's' then
      if nvl(p_indi_vali_limi_costo, 'N') = 'S' then
        I020009_A.pp_valida_limite_costo;
      end if;
  ...
  begin
    ...
    for x in c_hijos(p_movi_codi) loop
      I020009_A.pp_valida_movi_canc(x.movi_codi);
      I020009_A.pp_valida_cheq(x.movi_codi);
      for y in c_nietos(x.movi_codi) loop
        I020009_A.pp_valida_movi_canc(y.movi_codi);
        I020009_A.pp_valida_cheq(y.movi_codi);
        null;
      end loop;
    end loop;

    I020009_A.pp_valida_movi_canc(p_movi_codi);
    I020009_A.pp_valida_cheq(p_movi_codi);

    --validar el rango de fecha..
    for w in c_movi loop
      I020009_A.pp_valida_fech_modi(w.movi_fech_emis);
    end loop;
...
if  nvl(i_moco_impo_mone_ii,0) <=0 then
     raise_application_error(-20010,'El importe no puede quedar vacio o ser menor o igual a 0');
end if;
if  i_moco_tiim_codi   is null then
   ...
   if v('P74_S_TIMO_CALC_IVA') = 'N' and i_moco_impu_codi <> 1then
   raise_application_error(-20010,'Solo puede ser exento');
  end if;
    ...
    begin
      -- Call the procedure
      I020009_A.pp_muestra_come_conc(p_conc_codi           => i_moco_conc_codi,
    ...
    --raise_application_Error(-20001,'asdf');
      update come_tabl_auxi
         set taax_c001 = i_moco_conc_codi,
             ...
             taax_c029 = v_moco_grav10_ii_mmnn,
             taax_c030 = v_moco_grav5_ii_mmnn,
             taax_c031 = v_moco_grav10_mmnn,
             taax_c032 = v_moco_grav5_mmnn,
             taax_c033 = v_moco_iva10_mmnn,
             taax_c034 = v_moco_iva5_mmnn,
             taax_c035 = v_moco_exen_mmnn,
             taax_c036 = v_moco_impo_mmnn,
             taax_c037 = v_moco_impo_mone,


            taax_c039 = (select impu_desc from come_impu where impu_codi = i_moco_impu_codi),
             taax_c040 = i_moco_impu_codi,
             taax_c041 = i_moco_tiim_codi,
             taax_c042 = (select tiim_desc from come_tipo_impu where tiim_codi = i_moco_tiim_codi),
             taax_c043 = v('P74_JURI_NUME'),
             taax_c045 = v('P74_IMPO_CODI'),
             taax_c046 = v('P74_CECO_NUME'),
             taax_c047 = v('P74_EMSE_CODI'),
             taax_c048 = v('P74_MOCO_ORTR_CODI'),
             taax_c049 = v('P74_TRAN_CODI')
  ...
  select  p_session ,
    ...
    from (select
          ...
          substr(lpad(to_char(m.movi_nume), 13, '0'), 7,8)) movi_nume,
           TO_cHAR(m.movi_fech_emis,'DD/mm/yyyy') movi_fech_emis,
          m.movi_obse,
          o.oper_codi,
          o.oper_desc,
          ...
          ci.impu_codi,
          ci.impu_desc,
          cs.sucu_codi sucu_codi_1,
          cs.sucu_desc sucu_desc_2,
          cd.moco_fech_emis,
          case when m.movi_timo_codi = p_codi_timo_fcor then
               'FACTURA CONTADO RECIBIDA'
               when i_movi_timo_codi in (p_codi_timo_auto_fact_emit, p_codi_timo_auto_fact_emit_cr) then
                 'AUTOFACTURA EMITIDA'
            end titulo,
          'REPORTE' descr
    from come_movi m,
...
insert into come_tabl_auxi
  ...
  select
      ...
      p_usuario ,
       ...
       conc_codi,
       conc_desc,
         pack_nume_to_text.fa_conv_nume_text(deta_impo_mone) importe_letras,
       'REPORTE'
...
/*  v_cont := 'p_app_session:p_app_user';
  ...
  v_repo := 'I020013';*/
   ...
   --- commit;
  --RAISE_APPLICATION_ERROR(-20001,'REPORTE'||bcab.timo_dbcr_caja||'--'||bcab.movi_afec_sald||'**'||p_movi_codi);
...
Begin
  ...
  bcab.movi_grav10_ii_mmnn := round((bcab.movi_grav10_ii_mone*bcab.movi_tasa_mone),p_cant_deci_mmnn);
  bcab.movi_grav5_ii_mmnn  := round((bcab.movi_grav5_ii_mone*bcab.movi_tasa_mone), p_cant_deci_mmnn);

  bcab.movi_exen_mmnn      := round((bcab.movi_exen_mone*bcab.movi_tasa_mone), p_cant_deci_mmnn);
  ...
  bcab.movi_iva10_mmnn     := round((bcab.movi_grav10_ii_mmnn/11), p_cant_deci_mmnn);
  bcab.movi_iva5_mmnn      := round((bcab.movi_grav5_ii_mmnn/21), p_cant_deci_mmnn);
  ...
  bcab.movi_grav10_mmnn     := round((bcab.movi_grav10_ii_mmnn/1.1), p_cant_deci_mmnn);
  bcab.movi_grav5_mmnn      := round((bcab.movi_grav5_ii_mmnn/1.05), p_cant_deci_mmnn);
...
begin
  ...
  if bcab.clpr_indi_clie_prov = upper('p') then
    if bcab.s_clpr_codi_alte = p_codi_prov_espo then
      v_indi_espo := upper('s');
    end if;
  else
    if bcab.s_clpr_codi_alte = p_codi_clie_espo then
      v_indi_espo := upper('s');
    end if;
  end if;
  ...
  exception
    when others then
      raise_application_error(-20001,'Error, favor avisar al dpto de informatica');
  end;
  ...
  if nvl(v_tico_indi_timb_auto, 'N') = 'S' then
    if nvl(p_tico_indi_timb, 'C') = 'P' then
      if bcab.clpr_indi_clie_prov = upper('p') and
        ...
        for x in c_timb loop
          v_indi_entro := upper('s');
          if brete.movi_nume_timb_rete is not null then
            p_timb      := brete.movi_nume_timb_rete;
            p_fech_venc := brete.movi_fech_venc_timb_rete;
          else
            p_timb      := x.cptc_nume_timb;
            p_fech_venc := x.cptc_fech_venc;
          end if;
    ...
    if v_indi_entro = upper('n') then
      raise_application_error(-20001,'No existe registro de timbrado de retencion cargado para este proveedor, o el timbrado cargado se encuentra vencido!!!');
    end if;
  end if;
end;

procedure pp_cargar_timb_rete is
...
begin
    ...
    exception
      when no_data_found then
        raise_application_error(-20001, 'Tipo de comprobante no configurado!');
    end ;

  pp_validar_timbrado_rete (p_tico_codi_rete,
                            to_number(brete.s_nro_1_rete),
                            to_number(brete.s_nro_2_rete),
                            bcab.movi_clpr_codi,
                            bcab.movi_fech_emis,
                            brete.movi_nume_timb_rete,
                            brete.movi_fech_venc_timb_rete,
                            v_tico_indi_timb);
...
begin
  ...
  where secu_codi = (select peco_secu_codi
                     from come_pers_comp
                     where peco_codi = p_peco_codi)
    and secu_empr_codi = p_empr_codi;
...
procedure pp_cargar_nro_rete is
begin
   -- raise_application_error(-20001,'retencion'|| brete.movi_nume_rete);
 if brete.movi_nume_rete is null then
      pp_carga_secu_rete(brete.movi_nume_rete);
...
Procedure pp_muestra_clpr (p_indi_clpr         in varchar2,
                           ...
                           p_pais_codi         out number,
                           p_zona_codi         out number,  --zona de cliente
                           p_capr_codi         out number,   ---categoria proveedor
                           p_clpr_fech_regi    out date,
                           p_clpr_fech_modi    out date
                           ) is
begin
     select clpr_desc,
            clpr_codi,
            ...
            nvl(clpr_prov_retener, 'NO'), clpr_pais_codi,
            clpr_zona_codi,
            clpr_capr_codi,
            trunc(clpr_fech_regi),
            trunc(clpr_fech_modi)
     ...
     and clpr_indi_clie_prov = p_indi_clpr;
...
Exception
  ...
  procedure pp_calcular_retencion is
  --v_impo_iva  number;
  --v_impo_tota number;
  v_tota_mone number;
  v_tota_mmnn number;
...
begin
  ...
  if bcab.movi_timo_codi not in (p_codi_timo_fcor,p_codi_timo_pcor)
     or upper(nvl(bcab.clpr_prov_retener, 'SI')) = 'NO'
     or nvl(bcab.s_calc_rete, 'N') = 'N' then
      bcab.s_impo_rete       := 0;
      brete.movi_impo_rete   := 0;
  NULL;
  else


  v_tota_mone := nvl(bcab.s_total, 0);
  v_tota_mmnn := round(nvl(bcab.s_total, 0) * bcab.movi_tasa_mone, p_cant_deci_mmnn);

  if nvl(bcab.s_timo_calc_iva,'N') = 'S' then
    v_grav_mone := bcab.movi_grav_mone;
    v_grav_mmnn := bcab.movi_grav_mmnn;
    v_iva_mone  := bcab.movi_iva_mone;
    v_iva_mmnn  := bcab.movi_iva_mmnn;
    v_iva10_mone := bcab.s_iva_10;
    v_iva10_mmnn := round(bcab.s_iva_10 * bcab.movi_tasa_mone);
    v_iva5_mone := bcab.s_iva_5;
    v_iva5_mmnn := round(bcab.s_iva_5 * bcab.movi_tasa_mone);
  else --si es una boleta negra caso pandolfo
    v_grav_mone := nvl(bcab.s_total, 0) - round(nvl(bcab.s_total, 0) / 11, bcab.movi_mone_cant_deci);
    v_grav_mmnn := v_tota_mmnn - round(v_tota_mmnn / 11, p_cant_deci_mmnn);
    v_iva_mone  := round(nvl(bcab.s_total, 0) / 11, bcab.movi_mone_cant_deci);
    v_iva_mmnn  := round(v_tota_mmnn / 11, p_cant_deci_mmnn);
    v_iva10_mone := v_iva_mone;
    v_iva10_mmnn := v_iva_mmnn;
    v_iva5_mone  := 0;
    v_iva5_mmnn  := 0;
  end if;

  general_skn.pl_calc_rete (p_form_calc_rete_emit,
                bcab.movi_empr_codi,
                ...
                v_iva5_mone ,
                v_iva5_mmnn ,
                nvl(bcab.S_CALC_RETE_MINI,'S'));

  bcab.s_impo_rete     := nvl(bcab.s_impo_rete, 0);
 ...
 -- RAISE_aPPLICATION_ERROR(-20001,'VVV'||bcab.s_impo_rete||nvl(bcab.S_CALC_RETE_MINI,'S')||'---'||bcab.movi_mone_cant_deci);
  if nvl(bcab.s_impo_rete,0) > 0 then
    pp_cargar_nro_rete;
    pp_cargar_timb_rete;
  end if;
...
procedure pp_calcular_retencion_ivae is
  --v_impo_iva  number;
  --v_impo_tota number;
  v_tota_mone number;
  v_tota_mmnn number;
...
begin
  ...
  if bcab.movi_timo_codi not in (p_codi_timo_auto_fact_emit, p_codi_timo_auto_fact_emit_cr)
  or (upper(nvl(bcab.clpr_prov_retener, 'SI')) = 'NO' and nvl(bcab.s_calc_rete_ivae, 'N') = 'N')
  or nvl(bcab.clpr_pais_codi, 1) = 1 then
    if nvl(bcab.s_impo_rete,0) = 0 then
      bcab.s_impo_rete     := 0;
      brete.movi_impo_rete := 0;
    end if;
  ...
  v_porc_apli := p_porc_aplic_reten_iva_ext;

  if p_indi_apli_rete_exen = 'S' and nvl(bcab.clpr_indi_clie_prov, 'C') = 'P' then
    bcab.s_impo_rete     := round((v_tota_mone / 11), bcab.movi_mone_cant_deci);
    brete.movi_impo_rete := bcab.s_impo_rete;
  else
    bcab.s_impo_rete     := round((v_tota_mone * v_porc_apli / 100), bcab.movi_mone_cant_deci);
    brete.movi_impo_rete := bcab.s_impo_rete;
  end if;
...
end pp_calcular_retencion_ivae;

procedure pp_calcular_retencion_renta is
  --v_impo_iva  number;
  --v_impo_tota number;
  v_tota_mone number;
  v_tota_mmnn number;
...
begin
  ...
  if bcab.movi_timo_codi not in (p_codi_timo_auto_fact_emit, p_codi_timo_auto_fact_emit_cr)
  or (upper(nvl(bcab.clpr_prov_retener, 'SI')) = 'NO' and  nvl(bcab.s_calc_rete_rent, 'N') = 'N')
  or nvl(bcab.clpr_pais_codi, 1) = 1 then
    bcab.s_impo_rete_rent     := 0;
    brete.movi_impo_rete_rent := 0;
   NULL;
   else
  ...
  if nvl(bcab.s_abso_rete_rent, 'N') = 'S' then
    if p_indi_apli_rete_exen = 'S' and nvl(bcab.clpr_indi_clie_prov, 'C') = 'P' then
      v_tota_mone                := round((v_tota_mone / 1.1), bcab.movi_mone_cant_deci);
      v_porc_apli := p_porc_aplic_reten_rent_abso;
      bcab.s_impo_rete_rent     := round((v_tota_mone * v_porc_apli / 100), bcab.movi_mone_cant_deci);
      brete.movi_impo_rete_rent := bcab.s_impo_rete_rent;
    else
      v_porc_apli := p_porc_aplic_reten_rent_abso;
      bcab.s_impo_rete_rent     := round((v_tota_mone * v_porc_apli / 100), bcab.movi_mone_cant_deci);
      brete.movi_impo_rete_rent := bcab.s_impo_rete_rent;
    ...
    if bcab.movi_timo_codi in (p_codi_timo_auto_fact_emit, p_codi_timo_auto_fact_emit_cr) then
      if p_indi_apli_rete_exen = 'S' and nvl(bcab.clpr_indi_clie_prov, 'C') = 'P' then
        bcab.s_impo_rete_rent     := round((v_tota_mone / 11), bcab.movi_mone_cant_deci);
        brete.movi_impo_rete_rent := bcab.s_impo_rete_rent;
      else
        bcab.s_impo_rete_rent  := round(bcab.s_impo_rete, bcab.movi_mone_cant_deci);
      end if;
    else
      v_porc_apli := p_porc_aplic_reten_rent;
      bcab.s_impo_rete_rent     := round((v_tota_mone * v_porc_apli / 100), bcab.movi_mone_cant_deci);
      brete.movi_impo_rete_rent := bcab.s_impo_rete_rent;
    end if;
  end if;

  bcab.s_impo_rete_rent     := nvl(bcab.s_impo_rete_rent, 0);
  brete.movi_impo_rete_rent := bcab.s_impo_rete_rent;

  if nvl(bcab.s_impo_rete_rent,0) > 0 then
    pp_cargar_nro_rete;
    pp_cargar_timb_rete;
  end if;

 end if;
end pp_calcular_retencion_renta;


PROCEDURE pp_calcular_importe_cab_fp IS
BEGIN
    bcab.s_impo_cheq := -nvl(bfp.tota_cheq_egre_movi, 0) +
                         nvl(bfp.tota_cheq_ingr_movi, 0);
    bcab.s_impo_efec := nvl(bfp.tota_efec_ingr_movi, 0) -
                         nvl(bfp.tota_efec_egre_movi, 0);
    bcab.s_impo_tarj      := -nvl(bfp.tota_tarj_egre_movi, 0) +
                              nvl(bfp.tota_tarj_ingr_movi, 0);
    bcab.s_impo_adel      := -nvl(bfp.tota_adel_egre_movi, 0) +
                              nvl(bfp.tota_adel_ingr_movi, 0);
    bcab.s_impo_vale      := -nvl(bfp.tota_vale_egre_movi, 0) +
                              nvl(bfp.tota_vale_ingr_movi, 0);
    bcab.s_impo_rete_reci := nvl(bfp.tota_rete_reci_ingr_movi, 0) -
                              nvl(bfp.tota_rete_reci_egre_movi, 0);
    bcab.s_impo_viaj      := nvl(bfp.tota_viaj_ingr_movi, 0) -
                              nvl(bfp.tota_viaj_egre_movi, 0);

/*  -- pago
  if bcab.movi_timo_codi in
     (p_codi_timo_auto_fact_emit,
      p_codi_timo_auto_fact_emit_cr) then
    if p_indi_apli_rete_exen = 'S' and
       nvl(bcab.clpr_indi_clie_prov, 'C') = 'P' then
...
END;
procedure pp_calc_importe_cabe(p_devolver in varchar2) is
 begin
 ...
 --  pl_me(V('P74_CLPR_CODI_ALTE'));
  ...
  pp_calcular_importe_cab2;

  pp_calcular_retencion;

  pp_calcular_retencion_ivae;

  pp_calcular_retencion_renta;
  --se calcula de vuelta, despues de la retenci?n
  pp_calcular_importe_cab_fp;

setitem('P74_S_IMPO_RETE_RENT', bcab.S_IMPO_RETE_RENT);
setitem('P74_S_IMPO_RETE', bcab.S_IMPO_RETE);
setitem('P74_S_IMPO_ADEL',bcab.S_IMPO_ADEL);
setitem('P74_S_IMPO_CHEQ', bcab.S_IMPO_CHEQ);
setitem('P74_S_IMPO_EFEC', bcab.S_IMPO_EFEC);
setitem('P74_S_IMPO_TARJ', bcab.S_IMPO_TARJ);
setitem('P74_S_IMPO_VALE', bcab.S_IMPO_VALE);
setitem('P74_S_IMPO_VIAJ',bcab.S_IMPO_VIAJ);


 setitem('P74_MOVI_NUME_RETE',brete.movi_nume_rete);
setitem('P74_MOVI_NUME_TIMB_RETE',brete.movi_nume_timb_rete);
setitem('P74_MOVI_FECH_VENC_TIMB_RETE',brete.movi_fech_venc_timb_rete);
setitem('P74_S_MOVI_FECH_RETE',brete.movi_fech_rete);
setitem('P74_MOVI_IMPO_RETE',brete.movi_impo_rete);
setitem('P74_MOVI_IMPO_RETE_RENT',brete.movi_impo_rete_rent);

--RAISE_APPLICATION_eRROR(-20001,bcab.S_IMPO_RETE);
/*setitem('P74_S_TOTAL',bcab.);
setitem('P74_S_IVA_10', bcab.);
setitem('P74_S_TOT_IVA', bcab.S_TOT_IVA);

*/

end pp_calc_importe_cabe;


procedure pp_carga_secu (p_secu_nume_fact out number)is
  v_cant number;
begin
  /*if nvl(bcab.s_timo_calc_iva, 'N') = 'N' then --solo para fact en negro
    if nvl(parameter.p_indi_carg_secu_bole_pres, 'N') = 'S' and bcab.s_pres_codi is not null then
      p_secu_nume_fact  := bcab.pres_nume;
    else
  ...
  bcab.s_nro_1:=substr(lpad(p_secu_nume_fact,13,'0'),1,3);
  bcab.s_nro_2:=substr(lpad(p_secu_nume_fact,13,'0'),4,3);
  bcab.s_nro_3:=substr(lpad(p_secu_nume_fact,13,'0'),7,7);
  */
  select count(setc_nume_timb)
        into v_cant
        from come_secu_tipo_comp
       where setc_secu_codi =  (select user_secu_codi from segu_user a
                             where  a.user_login = gen_user)
         and setc_tico_codi = v('P74_TICO_CODI') --tipo de comprobante
         and setc_esta      = v('P74_S_NRO_1')      --establecimiento
         and setc_punt_expe = v('P74_S_NRO_2') --punto de expedicion
         and setc_fech_venc >= v('P74_MOVI_FECH_EMIS')
       order by setc_fech_venc;
  ...
  if nvl(V('P74_TIMO_INDI_CLPR'), 'N') = 'C' AND V('P74_MOVI_TIMO_CODI') IN (1,2) then

        begin
          select nvl(secu_nume_fact,0) + 1
            into p_secu_nume_fact
            from come_secu
           where secu_codi = (select user_secu_codi from segu_user a
                             where  a.user_login = gen_user);
 ...
 IF  V('P74_MOVI_TIMO_CODI') IN (9) THEN
...
exception
  when no_data_found then
    raise_application_error(-20001, 'Codigo de Secuencia de Facturacion inexistente');
...
begin
  ...
  /*if p_indi = 'S' then
    if nvl(bcab.s_timo_calc_iva, 'N') = 'N' then
      update come_secu set secu_nume_fact_negr = bcab.movi_nume
      where secu_codi = (select peco_secu_codi
                         from come_pers_comp
                         where peco_codi = parameter.p_peco_codi);
    ...
    if nvl(V('P74_TIMO_INDI_CLPR'), 'N') = 'C' AND V('P74_MOVI_TIMO_CODI') IN (1,2) then
      update come_secu set secu_nume_fact = bcab.movi_nume
      where secu_codi = (select user_secu_codi from segu_user a
                             where  a.user_login = gen_user);
...
procedure pp_validar_nro2 is
  v_nro_segun_fact number;
begin
 --raise_application_Error(-20001, TO_number(v('P74_MOVI_NUME'))||'-'||v_nro_segun_fact);
if v('P74_TIMO_INDI_CLPR') = 'C'  AND V('P74_MOVI_TIMO_CODI') IN (1,2,9) then
 I020009_A.pp_carga_secu(p_secu_nume_fact => v_nro_segun_fact);

  /*if  TO_number(v('P74_MOVI_NUME')) <> v_nro_segun_fact  then
      raise_application_Error(-20001,'El nro no corresponde a la secuencia del documento, puede utilizar :'||v_nro_segun_fact);
 end if;*/
end if;
...
procedure pp_imprimir_retencion (i_movi_codi in number) is
  ...
  if p_form_impr_rete = 1 then  --formato de impresion de rete. Pandolfo
  v_report := 'retencion_pand';
  insert into come_tabl_auxi
  (taax_sess,
   taax_user,
   taax_seq,
...
select  p_session,
        P_USUARIO,
        seq_come_tabl_auxi.nextval,
       to_char(m.movi_fech_emis,'dd') dia,
       to_char(m.movi_fech_emis, 'Month') mes,
       to_char(m.movi_fech_emis, 'yy') anho,
       nvl(m.movi_clpr_desc, cp.clpr_desc) razon_social,
       nvl(m.movi_clpr_ruc, cp.clpr_ruc) ruc,
       m.movi_obse,
       m.movi_mone_codi,
       fa_devu_fact_rete(m.movi_codi, 'nume') comprobantes,
       to_number(fa_devu_fact_rete(m.movi_codi, 'neto')) trans_sin_iva,
       to_number(fa_devu_fact_rete(m.movi_codi, 'iva')) trans_iva,
       to_number(fa_devu_fact_rete(m.movi_codi, 'total')) trans_tota,
       m.movi_exen_mone retencion,
       rpad(rtrim(ltrim(upper(pack_nume_to_text.fa_conv_nume_text(nvl(movi_exen_mone,0))))),244,'-') importe_letras,
       fa_busc_para('p_porc_aplic_reten') || ' %' porc_rete,
       'REPORTE' reporte
  from come_movi m, come_clie_prov cp
 where m.movi_clpr_codi = cp.clpr_codi(+)
   and m.movi_timo_codi =
       (select p.para_valo
          from come_para p
         where lower(p.para_nomb) = 'p_codi_timo_rete_emit')
     and movi_codi  =i_movi_codi;



  elsif p_form_impr_rete = 2 then  --Alpacasa
  	null;--no encontre reporte
  elsif p_form_impr_rete = 3 then  -- Sulmeta
     v_report := 'sulmetal_rete';
  insert into come_tabl_auxi
  (taax_sess,
   taax_user,
   taax_seq,
 ...
 where m.movi_clpr_codi = cp.clpr_codi(+)
   and m.movi_timo_codi =
       (select p.para_valo
         ...
         where lower(p.para_nomb) = 'p_codi_timo_rete_emit')
     and movi_codi  =i_movi_codi;
  elsif p_form_impr_rete = 4 then  -- Amambay
    v_report := 'retencion_amam';
     insert into come_tabl_auxi
      (taax_sess,
       taax_user,
       taax_seq,
...
select  p_session,
       ...
       m.movi_mone_codi,
       fa_devu_fact_rete(m.movi_codi, 'nume') comprobantes,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 1, 13) primero,
       'Factura',
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 15, 12) segundo,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 27, 12) tercero,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 39, 12) cuarto,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 51, 12) quinto,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 63, 12) sexto,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 75, 12) septimo,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 87, 12) octavo,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 99, 12) noveno,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 111, 12) decimo,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 123, 12) decimo_primero,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 135, 12) decimo_segundo,
       substr(fa_devu_fact_rete(m.movi_codi, 'nume'), 147, 12) decimo_tercero,
       to_number(fa_devu_fact_rete(m.movi_codi, 'neto')) trans_sin_iva,
       to_number(fa_devu_fact_rete(m.movi_codi, 'iva')) trans_iva,
       to_number(fa_devu_fact_rete(m.movi_codi, 'total')) trans_tota,
       m.movi_exen_mone retencion,
...
and movi_codi  =i_movi_codi;
null;
  elsif p_form_impr_rete = 5 then  -- Santaniana
    v_report := 'retencion_santa';
 insert into come_tabl_auxi
      (taax_sess,
       taax_user,
       taax_seq,
...
select  p_session,
 ...
 where m.movi_clpr_codi = cp.clpr_codi(+)
   and m.movi_timo_codi =
  ...
  elsif p_form_impr_rete = 6 then  -- Avenida
     v_report := 'retencion_ave';
    ...
    insert into come_tabl_auxi
      ...
      select p_session,
             P_USUARIO,
             seq_come_tabl_auxi.nextval,
             TO_char(m.movi_fech_emis, 'DD/mm/YYYY') fecha,
             nvl(m.movi_clpr_desc, cp.clpr_desc) razon_social,
             nvl(m.movi_clpr_ruc, cp.clpr_ruc) ruc,
             m.movi_obse,
             m.movi_mone_codi,
       ...
       where m.movi_clpr_codi = cp.clpr_codi(+)
         and m.movi_timo_codi =
             (select p.para_valo
               ...
               where lower(p.para_nomb) = 'p_codi_timo_rete_emit')
         and movi_codi = i_movi_codi;
  elsif p_form_impr_rete = 7 then  -- Tesaca_Estandar

  v_report := 'retencion_tesa_stand';
...
select p_session,
 ...
 where m.movi_clpr_codi = cp.clpr_codi(+)
  ...
  elsif p_form_impr_rete = 8 then  -- Tesaca_alpacasa
    v_report := 'retencion_tesa_alpa';
   insert into come_tabl_auxi
  (taax_sess,
   taax_user,
   taax_seq,
...
end pp_imprimir_retencion;


procedure pp_imprimir (p_movi_codi in number,
                       p_reporte in VARCHAR2 )is

begin

   if p_reporte ='FACT' then
     pack_repo_clie.pp_llamar_reporte_facturas(p_usuario => gen_user,
                                          p_codigo  => p_movi_codi);
   end if;

   if p_reporte ='RETE' then
     i020009_a.pp_imprimir_retencion(i_movi_codi => p_movi_codi);
   end if;

   if p_reporte ='OPAG' then
    i020009_a.pp_reporte_orden_pago(i_movi_codi => p_movi_codi);
   end if;
   if p_reporte ='MVAR' then
    I020009_A.pp_reporte(i_movi_codi      => p_movi_codi,
                         i_movi_timo_codi => 1);
   end if;
