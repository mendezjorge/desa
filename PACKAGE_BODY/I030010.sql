  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SKN"."I030010" is
  ...
  type r_parameter is record (
       ...
       p_mone_desc_abre_mmnn number := general_skn.fl_busca_parametro('p_mone_desc_abre_mmnn')
  );
  ...
  begin
    ...
    if i_mone_codi is null then
      o_mone_desc       := 'Consolidado en moneda Nacional';
	    --o_mone_desc_abre  := parameter.p_mone_desc_abre_mmnn;
	    --o_mone_cant_deci  :=  parameter.p_cant_deci_mmnn;	 
    ...
    if i_mone_codi is not null then
      ...
      for x in 1 .. v_cant_regi loop
        ...
        apex_collection.add_member(p_collection_name => parameter.collection_name,
                                   p_c001            => ta_cuen_banc(x).cuen_codi,
                                   p_c002            => ta_cuen_banc(x).cuen_desc,
                                   p_c003            => ta_cuen_banc(x).mone_desc_abre,
                                   p_c004            => v_sald_inic,
                                   p_c005            => v_debi,
                                   p_c006            => v_cred,
                                   p_c007            => v_sald_fini);
      ...
      for x in 1 .. v_cant_regi loop
        ...
        apex_collection.add_member(p_collection_name => parameter.collection_name,
                                   p_c001            => ta_cuen_banc(x)
                                                        .cuen_codi,
                                   p_c002            => ta_cuen_banc(x)
                                                        .cuen_desc,
                                   p_c003            => ta_cuen_banc(x)
                                                        .mone_desc_abre,
                                   p_c004            => v_sald_inic,
                                   p_c005            => v_debi,
                                   p_c006            => v_cred,
...
-----------------------------------------------
  ...
  begin
raise_application_error(-20010,'Debe iaaaaaaaaaaaaaaaaaaaentren imputables !!!');
    select cuco_codi, cuco_indi_impu, cuco_desc
      into p_codi, v_indi_impu, p_desc
      from come_cuen_cont
     where cuco_codi= p_codi;
     --cuco_nume = p_codi;--p_nume;
...
-----------------------------------------------
  ...
  begin
    ...
    if i_mone_codi is null then
      raise_application_error(-20010,'Debe de elegir una moneda!');
    end if;
...
-----------------------------------------------
  ...
  begin
    ...
    if i_cuco_codi is not null then
      p_where   := p_where||' and cuen_efec_cuco_codi = '|| i_cuco_codi;
      --add_parameter(pl_id,upper('p_cuco_nume'),text_parameter, :bsel.cuco_nume_efec);
      --add_parameter(pl_id,upper('p_cuco_desc_efec'),text_parameter, :bsel.cuco_desc_efec);
    end if;

    if i_mone_codi is not null then
      p_where   := p_where||' and cuen_mone_codi = '||to_char(i_mone_codi);
      /*add_parameter(pl_id,upper('p_indi_mmnn'),text_parameter, 'N');
    else
      add_parameter(pl_id,upper('p_indi_mmnn'),text_parameter, 'S');
      */
    end if;
    ...
    V_CONTENEDORES := 'p_fech_inic:p_fech_fini:p_where:p_empr_desc:p_mone_codi:p_mone_desc:p_cuen_indi_caja_banc_desc:p_cuco_nume:p_cuco_desc_efec';
...
-----------------------------------------------
procedure pp_muestra_come_cuen_cont_p(p_codi in number,
...
begin
  raise_application_error(-20010,
                          'Debe iaaaaaaaaaaaaaaaaaaaentren imputables !!!');
  select cuco_desc
    into p_desc
    from come_cuen_cont
   where cuco_codi = p_codi;
  ...
  if nvl(v_indi_impu, 'N') <> 'S' then
    raise_application_error(-20010,
                            'Debe ingresar solamente las cuentas contables que se encuentren imputables !!!');
  end if;
...
end pp_muestra_come_cuen_cont_p;
